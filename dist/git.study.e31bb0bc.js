...
var arr = enhancedArray([1, 2, 3, 4, 5]);
console.log(arr[-1]); //=> 5
...
function getRange(str) {
  var _str$split$map = str.split(':').map(Number),
      ...
      start = _str$split$map2[0],
      end = _str$split$map2[1];
...
function enhancedArray(arr) {
  return new Proxy(arr, {
    get: function get(target, property, receiver) {
      ...
      var indices = range ? range : getIndices(property);
      var values = indices.map(function (index) {
        var key = index < 0 ? String(target.length + index) : index;
...
//# sourceMappingURL=/git.study.e31bb0bc.js.map